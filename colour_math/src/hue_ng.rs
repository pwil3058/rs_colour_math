...
impl<F: ColourComponent> HueIfceTmp<F> for RGBHue {
    ...
    fn rgb_for_sum_and_chroma(&self, sum: F, chroma: F) -> Option<Result<RGB<F>, RGB<F>>> {
        ...
        if chroma == F::ZERO {
            ...
            Some(Ok(self.make_rgb(((chroma + other), other))))
        } else {
            None
        }
    }
...
impl<F: ColourComponent> HueIfceTmp<F> for CMYHue {
    ...
    fn rgb_for_sum_and_chroma(&self, sum: F, chroma: F) -> Option<Result<RGB<F>, RGB<F>>> {
        ...
        } else if sum > chroma * F::TWO && sum < F::THREE - chroma {
...
impl<F: ColourComponent> HueIfceTmp<F> for SextantHue<F> {
    ...
    fn rgb_for_sum_and_chroma(&self, sum: F, chroma: F) -> Option<Result<RGB<F>, RGB<F>>> {
        ...
        let mut rgb = if sum == F::ZERO {
            ...
            if components.0 > F::ONE || components.2 < F::ZERO {
                return None;
            };
            self.make_rgb(components)
        };
        ...
        if chroma < F::from(0.00001).unwrap() && chroma > F::ZERO {
            ...
            if !(xy.0 == F::ZERO && xy.1 == F::ZERO) {
                let rgb_second = crate::chroma::calc_other_from_xy_alt(xy);
                // deviation "second" indicates a drift in the hue
                if (rgb_second - self.1).abs() / rgb_second > F::from(0.000_000_000_1).unwrap() {
                    let value = (sum / F::THREE).min(F::ONE);
                    rgb = [value, value, value].into();
                }
            }
        };
        Some(Ok(rgb))
...
impl<F: ColourComponent> HueIfceTmp<F> for Hue<F> {
    ...
    fn rgb_for_sum_and_chroma(&self, sum: F, chroma: F) -> Option<Result<RGB<F>, RGB<F>>> {
        match self {
            ...
            Self::Other(sextant_hue) => sextant_hue.rgb_for_sum_and_chroma(sum, chroma),
        }
    }
}
...
mod hue_ng_tests {
            ...
            for chroma in &NON_ZERO_CHROMAS {
                for sum in &VALID_OTHER_SUMS {
                    if let Some(result) = hue.rgb_for_sum_and_chroma(*sum, *chroma) {
                        ...
                        assert!(
                            *sum <= min_shade_sum || *sum >= max_tint_sum,
                            "\n{} < {} < {} \n:: chroma: {} \nHue: {:?}",
                            min_shade_sum,
                            *sum,
                            max_tint_sum,
    ...
    fn secondary_rgb_for_sum_and_chroma() {
        for (hue, expected_mc_rgb) in Hue::<f64>::SECONDARIES
            .iter()
            .zip(RGB::<f64>::SECONDARIES.iter())
        {
            assert_eq!(hue.rgb_for_sum_and_chroma(0.0, 0.0), Some(Err(RGB::BLACK)));
