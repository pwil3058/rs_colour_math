...
fn primary_max_chroma_rgbs() {
    for (hue, expected_rgb) in Hue::PRIMARIES.iter().zip(RGB::<u64>::PRIMARIES.iter()) {
        assert_eq!(
            hue.max_chroma_rgb_for_sum(UFDRNumber::ONE).unwrap(),
            *expected_rgb
        );
        assert!(hue
            .max_chroma_rgb_for_sum::<u64>(UFDRNumber::ZERO)
            .is_none());
        assert!(hue
            .max_chroma_rgb_for_sum::<u64>(UFDRNumber::THREE)
            .is_none());
        for sum in [
        ...
        .iter()
        {
            ...
            match hue.try_max_chroma_rgb_for_sum::<u64>(*sum).unwrap() {
                Ok(rgb) => {
                    assert_eq!(rgb.sum(), *sum);
                    assert_eq!(rgb, expected);
                    assert_eq!(rgb.hue().unwrap(), *hue);
                }
                Err(rgb) => {
                    assert_eq!(rgb.sum(), *sum);
                    assert_eq!(rgb, expected);
        ...
        .iter()
        {
            ...
            match hue.try_max_chroma_rgb_for_sum::<u64>(*sum).unwrap() {
                ...
                Err(rgb) => {
                    assert_eq!(rgb.sum(), *sum);
                    assert_approx_eq!(rgb, expected, Prop(0x10000));
                    assert_approx_eq!(rgb.hue().unwrap(), *hue, Prop(0x10000));
                }
            }
        }
    }
    ...
    for (hue, expected_rgb) in Hue::SECONDARIES.iter().zip(RGB::<u64>::SECONDARIES.iter()) {
        ...
        .iter()
        {
            ...
            match hue.try_max_chroma_rgb_for_sum::<u64>(*sum).unwrap() {
                ...
                Err(rgb) => {
                    assert_eq!(rgb.sum(), *sum);
                    assert_approx_eq!(rgb, expected, Prop(0x100000));
                    assert_approx_eq!(rgb.hue().unwrap(), *hue, Prop(0x10000));
                }
            }
        }
    ...
    for sextant in Sextant::SEXTANTS.iter() {
        for item in SECOND_VALUES.iter() {
            ...
            for item in VALID_HUE_SUMS.iter() {
                ...
                match hue.try_max_chroma_rgb_for_sum::<u64>(sum).unwrap() {
                    Ok(rgb) => {
                        assert_eq!(sum, rgb.sum());
                        assert_eq!(rgb.chroma(), hue.max_chroma_for_sum(sum).unwrap());
                        assert_eq!(rgb.hue().unwrap(), hue);
                    }
                    Err(rgb) => {
                        assert_eq!(sum, rgb.sum());
...
fn lightest_darkest_hcv_for_chroma() {
    ...
    for (hue, hcv) in hues.iter().zip(hcvs.iter()) {
        assert_eq!(hue.try_lightest_hcv_for_chroma(Chroma::ZERO), None);
        assert_eq!(hue.try_darkest_hcv_for_chroma(Chroma::ZERO), None);
        assert_eq!(hue.try_lightest_hcv_for_chroma(Chroma::ONE), Some(Ok(*hcv)));
        assert_eq!(hue.try_darkest_hcv_for_chroma(Chroma::ONE), Some(Ok(*hcv)));
        for prop in SHADE_TINT_CHROMA_PROPS.iter().map(|f| Prop::from(*f)) {
            let shade_chroma = Chroma::Shade(prop);
            let darkest_shade = match hue.try_darkest_hcv_for_chroma(shade_chroma).unwrap() {
                Ok(hcv) => {
            ...
            let lightest_shade = match hue.try_lightest_hcv_for_chroma(shade_chroma).unwrap() {
                Ok(hcv) => {
                    assert_eq!(hcv.hue, Some(*hue));
                    assert_eq!(hcv.chroma(), shade_chroma);
                    hcv
            ...
            let darkest_tint = match hue.try_darkest_hcv_for_chroma(tint_chroma).unwrap() {
                Ok(hcv) => {
                    assert_eq!(hcv.hue, Some(*hue));
            ...
            let lightest_tint = match hue.try_lightest_hcv_for_chroma(tint_chroma).unwrap() {
                Ok(hcv) => {
                    assert_eq!(hcv.hue, Some(*hue));
                    assert_eq!(hcv.chroma(), tint_chroma);
                    hcv
...
fn lightest_darkest_rgb_for_chroma() {
    ...
    let hcvs: Vec<HCV> = HCV::PRIMARIES
        ...
        .collect();
    for (hue, hcv) in hues.iter().zip(hcvs.iter()) {
        assert_eq!(hue.try_darkest_rgb_for_chroma::<u64>(Chroma::ZERO), None);
        assert_eq!(hue.try_lightest_rgb_for_chroma::<u64>(Chroma::ZERO), None);
        assert_eq!(
            hue.try_darkest_rgb_for_chroma::<u64>(Chroma::ONE),
            Some(Ok(RGB::<u64>::from(hcv)))
        );
        assert_eq!(
            hue.try_lightest_rgb_for_chroma::<u64>(Chroma::ONE),
            Some(Ok(RGB::<u64>::from(hcv)))
        );
        for prop in SHADE_TINT_CHROMA_PROPS.iter().map(|f| Prop::from(*f)) {
            let shade_chroma = Chroma::Shade(prop);
            let darkest_shade = match hue.try_darkest_rgb_for_chroma::<u64>(shade_chroma).unwrap() {
                Ok(darkest_shade) => {
            ...
            let lightest_shade = match hue
                .try_lightest_rgb_for_chroma::<u64>(shade_chroma)
                .unwrap()
            {
                Ok(lightest_shade) => {
            ...
            assert!(darkest_shade.sum() < lightest_shade.sum());
            let tint_chroma = Chroma::Tint(prop);
            let darkest_tint = match hue.try_darkest_rgb_for_chroma::<u64>(tint_chroma).unwrap() {
                Ok(darkest_tint) => {
            ...
            let lightest_tint = match hue.try_lightest_rgb_for_chroma::<u64>(tint_chroma).unwrap() {
                Ok(lightest_tint) => {
    ...
    for sextant in Sextant::SEXTANTS.iter() {
        for item in SECOND_VALUES.iter() {
            ...
            for prop in SHADE_TINT_CHROMA_PROPS.iter().map(|f| Prop::from(*f)) {
                let shade_chroma = Chroma::Shade(prop);
                let darkest_shade =
                    match hue.try_darkest_rgb_for_chroma::<u64>(shade_chroma).unwrap() {
                        Ok(darkest_shade) => {
                            assert_eq!(darkest_shade.hue(), Some(hue));
                            darkest_shade
                        }
                ...
                let lightest_tint =
                    match hue.try_lightest_rgb_for_chroma::<u64>(tint_chroma).unwrap() {
                        Ok(lightest_tint) => {
                            assert_eq!(lightest_tint.hue(), Some(hue));
                            lightest_tint
                        }
...
//         assert_eq!(
//             hue.try_darkest_rgb_for_chroma::<f64>(Chroma::ONE),
//             Some(*expected_rgb)
//         );
//         assert_eq!(
//             hue.try_lightest_rgb_for_chroma::<f64>(Chroma::ONE),
//             Some(*expected_rgb)
//         );
//         let prop = Prop::from(0.5_f64);
//         let shade_chroma = Chroma::Shade(prop);
//         let shade = hue.try_darkest_rgb_for_chroma::<u64>(shade_chroma).unwrap();
//         let tint_chroma = Chroma::Tint(prop);
//         let tint = hue.try_lightest_rgb_for_chroma::<u64>(tint_chroma).unwrap();
